...
#
	...
	if !game_process_controller.current_game_process == game_process_controller.game_process.game_fight && abs(horizontal) > 0.2 || abs(vertical) > 0.2: # Set the deadzone threshold to 0.2 (adjust as needed)
		play("walk")
	else:
		play("idle_pistol")
